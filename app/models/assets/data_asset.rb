...
class DataAsset < Asset
  ...
  def self.generate_toc(assets)
    ...
    assets = assets.includes(:asset_urls)
    website = assets.first.website

    links_by_destination_id =
      Link.where(destination_id: assets.map(&:id))
          .includes(:source)
    ...
    File.open(toc_filename(website), "w") do |file|
      ...
      assets.sort_by { it.name.downcase }.each do |asset|
        referring_assets = (links_by_destination_id[asset.id] || [])
            .map(&:source)
            .uniq

        references = referring_assets.map do |referring_asset|
